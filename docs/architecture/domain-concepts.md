...
## Арматура И Другие Элементы На Линии

Арматура, фильтр, обратный клапан, счетчик, датчик, предохранительный клапан и похожие компоненты считаются оборудованием каталога.

Они не должны появляться только на чертеже. Если элемент влияет на схему, расчет, проверку или DXF, он должен быть `EquipmentDefinition` и размещенным `EquipmentInstance` с типом, системой, DN, позицией, источником и статусом проверки.

Текущий первый тип — категория `valve`. План, принципиальная схема, `EngineeringDrawing` и evidence report должны брать арматуру из `Project.equipmentInstances`, а не дорисовывать ее самостоятельно.

## DrawingModel / CadDrawing

`EngineeringDrawing` — промежуточная print-like модель листа для предпросмотра, SVG листа и будущего PDF. Она содержит sheet size, layers, drawing entities, title block и metadata.
...
`Project + equipmentDefinitions -> EngineeringDrawing -> CadDrawing -> AsciiDxfWriter`
...
`EquipmentInstance` с категорией `valve` / `sensor` / etc. для арматуры, датчиков и прочих элементов на линии
