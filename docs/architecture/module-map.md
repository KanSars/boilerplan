...
Для задач по реальному пилотному чертежу сначала читайте `docs/architecture/pilot-real-drawing-plan.md`. Этот файл задает текущий сквозной фокус, а карта ниже помогает найти конкретные модули.
...
- `src/domain/equipment/EquipmentDefinition.ts` — включает категорию `valve`; арматура, фильтры, датчики и т.п. должны быть обычными карточками оборудования, если отображаются или проверяются.
...
- `src/infrastructure/evidence/PilotDrawingEvidenceReportService.ts` — структурированный evidence/status report для pilot drawing; не выполняет ГОСТ-проверку.
- `src/infrastructure/evidence/MissingDataQuestionnaireService.ts` — превращает gaps из отчета в вопросы для заказчика/инженера и форматирует TXT-выгрузку.
...
- `src/shared/config/pilotKitSpecs.ts` — pilot pipe/valve specs с source/status для листа и отчета.
...
- `src/tests/drawingEvidenceReport.test.ts` — проверка отчета по источникам для pilot drawing.
- `src/tests/missingDataQuestionnaire.test.ts` — проверка опросника недостающих исходных данных.
...
- `src/domain/drawing/EngineeringDrawing.ts` — print-like drawing model для листа A3, preview и sheet SVG export.
- `src/features/boiler-room-editor/SheetDrawingPreview.tsx` — третий вид `Чертёж / Предпросмотр листа`.
- `src/infrastructure/drawing/BoilerRoomSheetDrawingService.ts` — детерминированная сборка пилотного листа: рамка, штамп, план, технологическая схема, DN, стрелки и арматура.
- `src/infrastructure/drawing/EngineeringDrawingToCadService.ts` — преобразование `EngineeringDrawing` в `CadDrawing`.
...
- `src/infrastructure/exporters/EngineeringSheetSvgExporter.ts` — SVG export листа из `EngineeringDrawing`.
...
Правило для агентов: `Project -> EngineeringDrawing -> CadDrawing -> Exporter/Writer`, не `React SVG -> DXF`.

Арматура и другие элементы на линии не должны создаваться внутри drawing service как невидимая графика. Сначала добавляйте их как `EquipmentDefinition` и `EquipmentInstance`, затем отображайте в плане, схеме, листе и evidence report.
