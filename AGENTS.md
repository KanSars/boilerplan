...
Ближайший сквозной фокус зафиксирован в `docs/architecture/pilot-real-drawing-plan.md`: реальный pilot-комплект оборудования должен проходить цепочку `EquipmentDefinition/ConnectionPoint -> EngineeringDrawing -> CadDrawing -> DXF -> evidence/status report`. Не уходите в соседние улучшения UI, если они не помогают этому пилоту.
...
- Арматура и другие проверяемые элементы должны быть обычными `EquipmentDefinition` / `EquipmentInstance` из каталога. Если элемент отображается на схеме/чертеже или участвует в проверках, он обязан существовать в `Project.equipmentInstances`, а не дорисовываться exporter/drawing service.
- `DrawingModel` / `EngineeringDrawing` / `CadDrawing` — промежуточные модели чертежа для preview, SVG листа и CAD/DXF export; это не React SVG и не DOM snapshot.
...
`Project -> EngineeringDrawing/DrawingModel -> CadDrawing -> Exporter/Writer`
